app.controller('usuariosController', function($scope, $route, $http) {	
	$scope.$route = $route;
	...
	$scope.getTipoIdentificacion = function() {
	    ...
	    .then(function(response) {
	    	$scope.itemsTipoIdentificacion = response.data;			    	
	    });			
	}
	...
	$scope.cargadatos = function(estado) {
		...
		if (estado) {
			var tipo = $scope.Usuarios.selectTipoIdentificacion[1];
			console.log($("#selectTipoIdentificacion option:selected").text());

			if (tipo == "201711121222515a08836b3d0eb") {
				$.blockUI({ css: { 
		            border: 'none', 
		            padding: '15px', 
		            backgroundColor: '#000', 
				...
				$http({
			        ...
			        method: "POST",
			        data: "consulta_ruc=" + "consulta_ruc" + "&txt_ruc=" + $scope.Usuarios.txt_2,
			        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
			        dataType: 'json',
			    })
			    .then(function (response) {
	...
	$scope.onChanged = function() {
		$http({
	        url: 'data/parametros/usuarios/app.php',
	        method: "POST",
	        data: "tipo=" + "cargarCiudades" +"&id=" + $scope.Usuarios.selectProvincia,
	        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
	    })
	    .then(function(response) {
	    	$scope.itemsCiudades = response.data;		 		    	    	
	    });
	}
	...
	$scope.getCargos = function() {
	    ...
	    .then(function(response) {
	    	$scope.itemsCargos = response.data;			    	
	    });			
	}
	...
	jQuery(function($) {				
	    ...
	    jQuery(grid_selector).jqGrid({	        
	        ...
	        colNames: ['ID','IDIDENTIFICACION','IDENTIFICACIÓN','NOMBRES COMPLETOS','FIJO','MOVIL','CIUDAD','DIRECCIÓN','CORREO','GENERO','CARGO','USUARIO','CLAVE','FOTO','VERIFICADOR','ESTADO'],
	        colModel:[      
	            ...
	            {name:'id_ciudad',index:'id_ciudad',frozen : true, hidden: true, align:'left',search:true,width: ''},
	...
	$scope.submitForm = function() {
		...
		formdata.append('tipoIdentificacion', undefinedFunction($scope.Usuarios.selectTipoIdentificacion));
		formdata.append('identificacion', undefinedFunction($scope.Usuarios.txt_2));
		formdata.append('nombres_completos', undefinedFunction($scope.Usuarios.txt_3));
		formdata.append('fijo', undefinedFunction($scope.Usuarios.txt_4));
		formdata.append('movil', undefinedFunction($scope.Usuarios.txt_5));
		formdata.append('direccion', undefinedFunction($scope.Usuarios.txt_6));
		formdata.append('ciudad', undefinedFunction($scope.Usuarios.selectCiudad));
		formdata.append('correo', undefinedFunction($scope.Usuarios.txt_7));
		formdata.append('genero', undefinedFunction($scope.Usuarios.selectGenero));
		formdata.append('cargo', undefinedFunction($scope.Usuarios.selectCargo));
		formdata.append('usuario', undefinedFunction($scope.Usuarios.txt_8));
		formdata.append('clave', undefinedFunction($scope.Usuarios.txt_9));
		formdata.append('confirmarClave', undefinedFunction($scope.Usuarios.txt_10));
		formdata.append('verificador', undefinedFunction($scope.Usuarios.txt_verificador));
		formdata.append('estado', undefinedFunction($scope.Usuarios.selectEstado));
